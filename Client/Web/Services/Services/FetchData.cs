...
public class FetchData : IFetchData
{
    ...
    public async Task FetchDataFromServerDb()
    {
        try
        {
            ...
            do
            {
                var staplesList = await _httpClient.GetFromJsonAsync<List<StaplesSourceDto>>($"{baseAddress}/api/FetchData/GetStaplesSourceItems/{startStapleCount}/{lastStapleCount}");
            ...
            do
            {
                var bellList = await _httpClient.GetFromJsonAsync<List<BellSourceDto>>($"{baseAddress}/api/FetchData/GetBellSourceitems/{startBellCount}/{lastBellCount}");
